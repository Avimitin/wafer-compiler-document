...
课题二的前端流程展示主要分成三步：首先展示算子库，让用户知悉目前有哪些算子可以使用，算子具体的功能和信息。
接下来进入代码编辑阶段，这部分则按照 VSCode 已有的界面制作即可。
最后在用户点击编译之后，进入一个新的前端展示页面，利用编译器编译完所提供的信息，
展示当前编写程序的任务划分和算子映射。
算子映射的部分之后也可以和课题三的核组库结合一起展示。
...
#figure(
  image("./assets/kernel.drawio.png"),
  caption: [
    算子库示意
  ]
)
...
为了更清晰地阐述这些目标，以下将按照这三个方面分别叙述课题二对前端部分的设想。
...
#figure(
  image("./assets/frontend-design.png"),
  caption: [
    流程图示意
  ],
)
