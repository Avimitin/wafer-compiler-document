...
本篇文档主要讲述课题二对于算法前端展示的一些初步设想，旨在为课题五的方案实现提供可行性分析。初稿涵盖了已有的密码学部分的实现，神经网络部分的内容将在后续进行补充和完善。

== 密码学前端设计

密码学部分的前端展示主要需要达成算法的分解、部署和映射等目标。为了更清晰地阐述这些目标，以下将按照这三个方面分别叙述课题二对密码学前端部分的设想。

首先，用户可以在前端提供的编辑器上编写代码并选择编译方式。在代码编写完成后，用户可以通过一个简单易用的按钮，将
`args.json` 的参数以表单的形式呈现给用户进行选择。前端将根据用户的选择重新序列化为 JSON 文件，以便提供给编译器进行后续处理。
...
在密码学部分的优化实现中，我们主要关注如何将大量独立输入按照负载比例分成不同的任务组，并在不同的核组上并行运行。这样的设计不仅提高了算法的执行效率，也充分利用了硬件资源。
